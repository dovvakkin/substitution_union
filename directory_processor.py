...
def train(word_data, EPOCHS, N_ANCH, n_rand, n_neg_per, triplet_alpha, alpha_mul, alpha_pow):
    ...
    os.mkdir('/home/y.kozhevnikov/run_tak/{}'.format(name))
    ...
    with open("/home/y.kozhevnikov/run_tak/{}/left.txt".format(name), 'w') as f:
        for l in __left:
            f.write("{}\n".format(l))
    with open("/home/y.kozhevnikov/run_tak/{}/right.txt".format(name), 'w') as f:
        for r in __right:
            f.write("{}\n".format(r))
    with open("/home/y.kozhevnikov/run_tak/{}/loss.txt".format(name), 'w') as f:
        for l in __loss:
            f.write("{}\n".format(l))


    for n, t in enumerate(__tensors_mul):
        torch.save(t, "/home/y.kozhevnikov/run_tak/{}/mul_{}.pt".format(name, n))
    for n, t in enumerate(__tensors_pow):
        torch.save(t, "/home/y.kozhevnikov/run_tak/{}/pow_{}.pt".format(name, n))
...
def unite_with_params(params, alpha, dfs, un_pairs):
    ...
    for context in tqdm_notebook(dfs[0]['context']):
        ...
        res = list([(k,v) for k,v in zip(united_probs[inds], np.array(words)[inds])])
        
#         print(new['substs_probs'])
#         print(res)
...
def save_subst(df, fname):
    ...
    df[['context_id','word','gold_sense_id','predict_sense_id','positions','context','word_at']].to_csv(fname + '.input', index=False)

def get_s_p(line, vocab):
    vocab = {vocab[k]:k for k in vocab}
    l = []
    for i in range(line.shape[0]):
        l.append((float(line[i]), vocab[i]))
    l.sort(reverse=True)
    return [l]

def get_united(word_data, data, per_word_vocabulary, m, p, df):
    df = df.copy()
    df = df[df['word'].isin(data)]
    for word in data:
        _, _, probs = word_data[word]
                
        fin = (probs.pow(p) * m).sum(0)
        
        for context, line in zip(df[df['word'] == word]['context'], fin):
            df.loc[df['context'] == context, ['substs_probs']] = get_s_p(line, per_word_vocabulary[word])
            
    return df


train = ['лавка', 'лайка', 'лев', 'лира', 'мина', 'мишень', 'обед', 'оклад', 'опушка', 'полис']
test = ['балка', 'вид', 'винт', 'горн', 'губа', 'жаба', 'клетка', 'крыло', 'купюра', 'курица', 'пост', 'поток', 'проказа', 'пропасть', 'проспект', 'пытка', 'рысь', 'среда', 'хвост', 'штамп']
...
for n in range(19):
    muls.append(torch.load(path + 'mul_{}.pt'.format(n)))
    pows.append(torch.load(path + 'pow_{}.pt'.format(n)))

for n, (m, p) in enumerate(zip(muls, pows)):
    un_train = get_united(new_data, train, per_word_vocab, m, p, df)
    un_test = get_united(new_data, test, per_word_vocab, m, p, df)

    save_subst(un_train, path + 'train/{}.npz'.format(n))
    save_subst(un_test, path + 'test/{}.npz'.format(n))
